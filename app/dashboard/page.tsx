...
import AttendanceRanking from "../components/attendance-ranking"
...
const calculateAttendanceStats = (attendanceRecords: AttendanceWithEvent[], userJoinDate?: string) => {
  ...
  for (const record of pastEvents) {
    ...
    } else if (record.actual_status === 'absent') {
      console.log('불참 이벤트 발견, 연속 출석 중단:', record.event?.title)
      break // 불참이 나오면 연속 출석 중단
    }
    // actual_status가 'unknown'인 경우는 무시하고 계속 진행
  }
  ...
  console.log('=== 최종 계산 결과 (입단일 기준) ===', result)
  return result
}
          ...
          <CardContent className="space-y-4">
            {isLoading ? (
              ...
              <>
                ...
                {/* 출석률에 따른 메시지 */}
                <div className="text-center p-2 rounded-lg bg-gradient-to-r from-gray-800 to-gray-700">
                  <p className="text-sm text-gray-300">
                    {attendanceStats.attendanceRate >= 90 ? "🏆 최고의 출석률을 보여주고 있어요!" :
                     attendanceStats.attendanceRate >= 70 ? "👏 좋은 출석률을 유지하고 있어요!" :
                     attendanceStats.attendanceRate >= 50 ? "📈 출석률을 더 높여보세요!" :
                     "💪 다음 모임에는 꼭 참석해주세요!"}
                  </p>
                </div>

                {/* 관리자 페이지 버튼 - 매니저인 경우만 표시 */}
                {user.role === "manager" && (
                  <Button
                    onClick={() => setCurrentView("admin")}
                    className="w-full bg-purple-700 hover:bg-purple-600 text-white justify-between mt-4"
                  >
